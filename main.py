...
TOKEN = "7939469386:AAEHHy4DwLLph67JtbD_eIgMUwFKpz8dEX8"
...
async def show_scoreboard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for uid, data in user_stats.items():
        if chat_id in data["groups"]:
            temp.append((uid, data["name"], data["groups"][chat_id]))
...
async def show_global_top(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    temp = [(uid, data["name"], data["total"]) for uid, data in user_stats.items()]
    temp.sort(key=lambda x: x[2], reverse=True)
    max_score = temp[0][2] if temp else 1
